...
class AIAlphaBacktester:
                ...
                if p_resp and p_resp.status_code == 200:
                    ...
                    if status == 'COMPLETE':
                        ...
                        if a_resp:
                            ...
                            logger.info(f"FINAL | {desc} | Sharpe: {res['s']:.2f} | Fit: {res['f']:.2f} | SC: {res['sc']:.4f}")
                            self.save_result(expr, res, desc, settings)
                            
                            if res['s'] >= 1.25 and res['f'] >= 1.1 and res['sc'] <= 0.5 and abs(res['sub_s']) >= dynamic_thresh:
                                logger.info(f"★★★ [GOLDEN RA FOUND] ★★★ : {desc} (SC SAFE: {res['sc']:.4f})")
                                self.save_golden(expr, res, desc, settings)
                            elif res['sc'] >= 0.7:
                                logger.warning(f"⚠️ [HIGH SC] : {desc} SC is {res['sc']:.4f}")
                            return res
                        return None
                    elif status == 'ERROR':
                        logger.warning(f"SIM ERROR for {desc}")
                        return None
        return None
    ...
    def process_item(self, row):
        try: self.run_simulation(row['expression'], row['desc'])
        except Exception as e: logger.error(f"Worker Error for {row['desc']}: {e}")
